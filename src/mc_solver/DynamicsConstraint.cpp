                                      ...
                                      double timeStep)
{
  ...
  if(robot.flexibility().size() != 0)
  {
    ...
    motionConstr.reset(new tasks::qp::MotionSpringConstr(robots.mbs(), static_cast<int>(robotIndex), robot.fd(),
                                                         tBound, tDBound, timeStep, sjList));
  }
  /*FIXME Implement?
  ...
  {
    motionConstr.reset(new tasks::qp::MotionConstr(robots.mbs(), static_cast<int>(robotIndex), robot.fd(),
                                                   tBound, tDBound, timeStep));
  }
}
